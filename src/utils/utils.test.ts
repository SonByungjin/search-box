...
describe('util 함수 테스트', () => {
    ...
    test('uri 경로에 따라 해당 값이 실제로 존재 하는가', () => {
        ...
        expect(checkItemPath(utils.createUri(testStringUri, mockItem))).toEqual(testStringUri);
    });
    ...
    test('자동 완성 배열의 요소 개수가 실제 데이터 매칭 개수와 일치하는가', () => {
        ...
        const testStringAutoCount = mockItemFilterd.searchable.filter((search) => search.includes(testStringAuto)).length;
        expect(utils.filterAutoCompleteItems([mockItemFilterd], testStringAuto).length).toEqual(testStringAutoCount);
    });
